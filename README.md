...
在编译内核后，CI 工作流会自动使用 QEMU 进行启动测试，以确保内核不会出现启动循环问题。

**测试内容：**
1. 自动下载 [Lineage OS QEMU 镜像](https://github.com/jqssun/android-lineage-qemu)
2. 使用 `qemu-system-aarch64` 启动编译好的内核
3. 监测内核启动过程，检测以下情况：
   - ✓ 成功启动 - 检测到系统初始化
   - ✗ 内核崩溃 - 检测到 Kernel panic
   - ✗ 启动循环 - 检测到多次重启
4. 测试结果会在 Actions 日志中显示

**特点：**
- 测试设置为 `continue-on-error: true`，即使测试失败也不会阻止构建完成
- 测试超时时间为 2 分钟，适合大部分内核启动场景
- 详细的日志输出，便于排查问题

**注意事项：**
- 测试需要下载约 956MB 的 VM 镜像，会增加 CI 运行时间
- 测试不确定性结果不代表内核有问题，可能需要更长时间才能完全启动
- 建议在本地也进行测试，确保内核在实际设备上的兼容性
