...
/*
 * This module (shadow_overlay2.ko) is the genuine, vendored-from-upstream
 * fs/overlayfs "overlay" filesystem — NOT a simulation. It exists because
 * production GKI boot images generally ship with CONFIG_OVERLAY_FS disabled,
 * which breaks dockerd's overlay2 graphdriver; loading this module registers a
 * working "overlay" filesystem type. It only compiles against the
 * android14-6.1 kernel branch (the vfs_tmpfile_open/lookup_one/mnt_idmap ABIs
 * differ on other KMIs). See README.md.
 *
 * Unlike when these sources were linked into the old combined shadow_ctr.ko
 * (which carried the module-wide metadata once, in shadow_ctr_main.c), this is
 * now a standalone .ko, so it carries its own MODULE_* metadata and
 * module_init()/module_exit() wiring (see the bottom of this file).
 */
 ...
 * ovl_exit() to avoid symbol collisions with a builtin overlayfs, and wired
 * up as this standalone module's init_module()/cleanup_module() aliases via
 * module_init()/module_exit() at the very bottom of this file.
 */
...
#define SHADOW_OVERLAY2_VERSION "2.0"

/*
 * Presence marker for shadow_ctr_checker (see shadow_sysvipc.c for rationale).
 * The checker also independently queries get_fs_type("overlay") as the
 * ground-truth "is mount(2) going to work" signal; this symbol specifically
 * answers "is the shadow_overlay2.ko module loaded".
 */
int shadow_overlay2_is_active(void)
{
	return 1;
}
EXPORT_SYMBOL_GPL(shadow_overlay2_is_active);

module_init(shadow_overlay_init);
module_exit(shadow_overlay_exit);

MODULE_LICENSE("GPL v2");
MODULE_AUTHOR("GKI_KernelSU_SUSFS contributors");
MODULE_DESCRIPTION("Vendored fs/overlayfs (android14-6.1) packaged as a standalone loadable \"overlay\" filesystem module for GKI images shipped without CONFIG_OVERLAY_FS");
MODULE_VERSION(SHADOW_OVERLAY2_VERSION);
