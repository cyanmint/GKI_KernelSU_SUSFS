...
`shadow_sysvipc` is a **standalone, independently loadable** module
(`shadow_sysvipc.ko`) — one of the modules in the `shadow_ctr/` family (see
`../README.md` for the umbrella overview). It keeps enough SysV IPC bookkeeping
alive on a kernel built **without `CONFIG_SYSVIPC`** for container runtimes to
