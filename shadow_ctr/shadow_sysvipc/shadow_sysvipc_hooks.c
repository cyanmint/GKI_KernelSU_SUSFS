...
/*
 * Presence marker for shadow_ctr_checker. The checker does symbol_get() on
 * this symbol (and symbol_put() afterwards) to detect at runtime whether this
 * module is loaded, without a build-time (KBUILD_EXTRA_SYMBOLS) dependency
 * edge — so the checker builds and loads standalone regardless of which subset
 * of shadow modules is present. Returns the module ABI/version tag.
 */
