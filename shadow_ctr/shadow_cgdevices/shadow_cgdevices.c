 ...
 *
 * A standalone loadable kernel module that retains the transparent device-open
 * hook points used by the shadow_ctr family on kernels without
...
MODULE_DESCRIPTION("Simulated cgroup device-access controller hook shim with transparent device-open hijacking only");
