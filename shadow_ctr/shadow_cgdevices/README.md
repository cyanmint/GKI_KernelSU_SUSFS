...
`shadow_cgdevices` is a **standalone, independently loadable** module
(`shadow_cgdevices.ko`) — one of the modules in the `shadow_ctr/` family (see
`../README.md` for the umbrella overview). It now only retains the transparent
device-open hook points; the old misc-device/ioctl-configured shadow rule store
was removed. It has no dependency on any other shadow module.
...
```sh
cd shadow_ctr/shadow_cgdevices
