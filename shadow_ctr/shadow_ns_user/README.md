...
`shadow_ns_user.ko` is a **thin user namespace presence/extension
module** for `shadow_ns_base.ko`.

## What it is — and is not

It is **honestly bookkeeping-only**. Loading it does **not** add any real
user namespace isolation. All the actual shadow-namespace machinery — the
...
1. **Explicit enablement.** Loading it is a deliberate statement that the
   deployer has chosen to "enable" the user (`SHADOW_NS_TYPE_USER`) shadow
   namespace type. It is easy to see with `lsmod` which types were switched on.
2. **Extension point.** It claims the per-type plugin slot for `SHADOW_NS_TYPE_USER`,
