...
`shadow_ctr.ko` (see `../README.md` for the umbrella overview). It supplies a
...
* `shadow_mqueue` does **not** register a `"mqueue"` filesystem type of its
  ...
  `insmod: failed to load shadow_ctr.ko: No such file or directory`
...
`mount("mqueue", "/dev/mqueue", ...)` *after* `shadow_ctr.ko` is loaded.
