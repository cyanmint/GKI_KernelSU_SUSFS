...
/*
 * shadow_hijack - shared hook engine linked into the unified shadow_ctr.ko.
 *
 * Background
 * ----------
 ...
 * called from the unified shadow_ctr.ko entry/exit sequencer. Callers describe
 ...
 * caller and the hook engine all live inside the same shadow_ctr.ko, so
...
static int shadow_hook_pre_handler(struct kprobe *p, struct pt_regs *regs)
{
	 ...
	 * instructions have executed. hook->owner is the merged shadow_ctr.ko
