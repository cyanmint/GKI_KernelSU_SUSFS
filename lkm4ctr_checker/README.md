...
ns_user (id mapping)         SKIP  running as uid 0; test needs a non-root uid to be conclusive
...
| `ns_pid (/proc stat content)` | From a fresh pid-namespace init (after mounting a fresh `/proc`): reads `/proc/self/stat` and `/proc/self/status` and parses the `pid`/`ppid`/`pgrp`/`session` fields and `Pid:`/`PPid:` lines; PASS only if **all** of them report the virtualized values (`pid=1 ppid=0 pgrp=1 session=1`), directly exercising `shadow_ns_procfs.c`'s `read`/`pread64` content-rewriting hooks rather than just checking directory-entry visibility. |
...
| `ns_pid (pgid/session)` | From the namespace's own pid-1 task: `getpgid(0)`/`getsid(0)` should both report `1` and `setpgid(0, 0)` should succeed, consistent with the `ns_pid (PID)` vpid remap. |
