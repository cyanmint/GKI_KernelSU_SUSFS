# shadow_configz — /proc/config.gz spoofing overlay

`shadow_configz` is a standalone loadable kernel module that overlays
`/proc/config.gz` with a build-time-generated, gzip-compressed configuration
text advertising the `CONFIG_*` symbols container runtimes probe for
...
even though the [`shadow_hook`](../shadow_hook)-based `shadowns` /
`shadow_sysvipc` / `shadow_mqueue` / `shadow_cgdevices` modules already
transparently provide (partial, honestly-documented) behaviour for the
corresponding subsystem.

`shadow_configz` makes the kernel's self-report to userspace consistent with
what those modules actually provide, so unconditional `CONFIG_*` preflight
...
behaviour by itself. Only load it together with the shadow module(s) that
back the `CONFIG_*` symbol(s) you are spoofing — e.g. spoofing
`CONFIG_SYSVIPC=y` without `shadow_sysvipc.ko` loaded just turns a clear
...
Note that even with a shadow module loaded, some of the spoofed symbols only
have *partial* fidelity (see each module's own README's "Honest scope /
limitations" section, e.g. `shmat`/`shmdt` in `shadow_sysvipc` remain
unimplemented, `shadow_cgdevices` enforcement requires an explicit bind
...
unloading `shadow_configz` simply leaves `/proc/config.gz` absent, matching
...
| `shadow_configz.c`     | The kernel module.                                      |
| `Makefile`             | Out-of-tree build (`make KDIR=...`), also regenerates `config_gz_data.h`. |
| `Kbuild`               | In-tree build fragment (`CONFIG_SHADOW_CONFIGZ`).       |

## Building

```sh
cd ctr_patches/shadow_configz
make KDIR=/path/to/kernel/build ARCH=arm64 LLVM=1
sudo insmod shadow_configz.ko
zcat /proc/config.gz | grep CONFIG_SYSVIPC
```

## Loading order

Load `shadow_configz.ko` alongside whichever of `shadowns.ko` /
`shadow_sysvipc.ko` / `shadow_mqueue.ko` / `shadow_cgdevices.ko` you are
using; load order between them does not matter (they are fully independent
modules).
