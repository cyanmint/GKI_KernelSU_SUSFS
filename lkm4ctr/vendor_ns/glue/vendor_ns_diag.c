...
size_t vendor_ns_diag_snprintf(char *buf, size_t buflen)
{
	...
	pos += scnprintf(buf + pos, buflen - pos,
		...
		vendor_ns_registry.stat_unshare, vendor_ns_registry.stat_setns,
		vendor_ns_registry.stat_clone, vendor_ns_registry.stat_uts_set,
		vendor_ns_registry.stat_uts_get, vendor_ns_registry.stat_pid_xlate,
		vendor_ns_registry.stat_uid_xlate, vendor_ns_registry.stat_proc_filtered);
	pos += scnprintf(buf + pos, buflen - pos, "namespaces by type:\n");
	for (type = 0; type < VENDOR_NS_TYPE_MAX; type++)
		pos += scnprintf(buf + pos, buflen - pos,
			"  %-6s live=%lu created=%lu  [%s]\n",
			vns_type_name(type), vendor_ns_registry.ns_count[type],
			vendor_ns_registry.ns_created[type], vns_type_behaviour(type));
...
size_t vendor_ns_diag_snprintf_type(u32 type, char *buf, size_t buflen)
{
	...
	pos += scnprintf(buf + pos, buflen - pos,
		...
		vendor_ns_registry.ns_count[type], vendor_ns_registry.ns_created[type]);
	list_for_each_entry(entry, &vendor_ns_registry.ns_list[type], link) {
		struct ns_common *ns = entry->ns;
		pos += scnprintf(buf + pos, buflen - pos,
			"  inum=%u refs=%u\n", ns->inum, refcount_read(&ns->count));
		if (type == VENDOR_NS_TYPE_UTS) {
			struct uts_namespace *uts = container_of(ns, struct uts_namespace, ns);
			pos += scnprintf(buf + pos, buflen - pos,
				"    nodename=\"%s\" domainname=\"%s\"\n",
				uts->name.nodename, uts->name.domainname);
		} else if (type == VENDOR_NS_TYPE_USER) {
			struct user_namespace *u = container_of(ns, struct user_namespace, ns);
			pos += scnprintf(buf + pos, buflen - pos,
				"    level=%d uid_extents=%u gid_extents=%u\n",
				u->level, u->uid_map.nr_extents, u->gid_map.nr_extents);
		} else if (type == VENDOR_NS_TYPE_PID) {
			struct pid_namespace *p = container_of(ns, struct pid_namespace, ns);
			pos += scnprintf(buf + pos, buflen - pos,
				"    level=%u pid_max=%d adding=%d cursor=%u\n",
				p->level, vns_get_pid_max(), !!(p->pid_allocated & PIDNS_ADDING),
