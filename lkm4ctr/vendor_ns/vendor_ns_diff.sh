#!/bin/sh
# vendor_ns_diff.sh - diff vendored files against kernel-common originals.
# Usage: ./vendor_ns_diff.sh [kernel-common-path]
# Default kernel path: $RUNNER_TEMP/kernel-common

set -e
KERNEL_BASE="${1:-${RUNNER_TEMP:-/home/runner/work/_temp}/kernel-common}"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
VENDOR_NS_DIR="$SCRIPT_DIR"
...
diff_file() {
    rel_path="$1"
    vendored="$VENDOR_NS_DIR/$rel_path"
